...
struct io_context {
    ...
    io_context_t *next;
    HAPTime wakeup_time; // Wakeup after `wakeup_time` Milliseconds.
...
compute_wakeup_time(HAPTime now)
{
    ...
    if (next != 0) {
        ...
        start_timer(otThreadInstance, s_timer, milliseconds);
        DEBUG("SRP client timer restarted at %d, %p", milliseconds, s_timer);
    }
}
...
{
    io_context_t *io_context;
    HAPTime now = otPlatAlarmMilliGetNow(), next = 0;
    bool more;
...
srp_connect_udp(void *context, const uint8_t *port, uint16_t address_type, const uint8_t *address, uint16_t addrlen)
{
    ...
    if (err == kDNSServiceErr_NoError) {
        ...
        io_context->sockaddr.mPort = ((uint16_t)(port[0]) << 8) + port[1];
...
{
    INFO("Register Reply: %d " PRI_S_SRP " " PRI_S_SRP " " PRI_S_SRP "\n", errorCode, name == NULL ? "<NULL>" : name,
...
srp_thread_init(otInstance *instance)
{
    ...
    s_timer = create_timer(otThreadInstance, wakeup_callback);
    if (s_timer == NULL) {
        ERROR("failed to create SRP client timer");
        return kDNSServiceErr_Unknown;
    }
    return kDNSServiceErr_NoError;
}
...
srp_thread_shutdown(otInstance *instance)
{
    ...
    stop_timer(otThreadInstance, s_timer);
